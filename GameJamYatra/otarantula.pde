class otarantula
{
  ...
  PImage otarantula[] = new PImage[COUNT];
  int Scene = 1;
  ...
  otarantula()
  {
    for (int i = 0; i<COUNT; i++)
    {
      ...
      speedX[i] = random(-1, 1);
      speedY[i] = random(-1, 1);
      // otarantula[i] = loadImage("otarantula.png");
      // otarantula[i].resize(90, 90);
    }
  }
  void Render()
  {
    if (Scene == 1)
    {
      ...
      textSize(100);
      text("Press RIGHT to continue", 100, 100);
      if (keyPressed)
      {
        if (key == 'f')
        {
